...
import { BatchStatus, CertificationStatus, ElectricStatus, NotificationKind, ShipmentStatus } from '@prisma/client';
...
export class NotificationsService {
  ...
  async runScheduledChecks() {
    ...
    const dedupe = async (title: string) =>
      (await this.prisma.notification.count({ where: { title, createdAt: { gte: today } } })) > 0;
    ...
    for (const b of overdueBatches) {
      ...
      if (!(await dedupe(title))) {
        await this.prisma.notification.create({
          data: { kind: NotificationKind.TREATMENT_FINISHED, title, link: `/treatments/${b.id}` },
        });
      }
    }
    ...
    const dueElectric = await this.prisma.electricTreatment.findMany({
      where: { status: ElectricStatus.RUNNING, estimatedFinish: { lt: new Date() } },
    ...
    for (const e of dueElectric) {
      ...
      if (!(await dedupe(title))) {
        await this.prisma.notification.create({
          data: { kind: NotificationKind.ELECTRIC_DUE, title, link: `/treatments/electric` },
        });
      }
    }
    ...
    const staleCerts = await this.prisma.certification.count({
      where: { status: CertificationStatus.SENT_TO_LAB, createdAt: { lt: cutoff } },
    });
    if (staleCerts > 0) {
      const title = `${staleCerts} certificate(s) pending at labs for over 14 days`;
      if (!(await dedupe(title))) {
        await this.prisma.notification.create({
          data: { kind: NotificationKind.CERTIFICATE_MISSING, title, link: `/certifications` },
        });
      }
    }
    ...
    const staleShipments = await this.prisma.exportShipment.count({
      where: { status: { in: [ShipmentStatus.DRAFT, ShipmentStatus.PENDING] }, createdAt: { lt: new Date(Date.now() - 7 * 86_400_000) } },
    });
    if (staleShipments > 0) {
      const title = `${staleShipments} export shipment(s) pending for over 7 days`;
      if (!(await dedupe(title))) {
        await this.prisma.notification.create({
          data: { kind: NotificationKind.EXPORT_PENDING, title, link: `/exports` },
        });
      }
    }
  }
