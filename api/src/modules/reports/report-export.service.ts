...
export class ReportExportService {
  sendCsv(res: Response, report: TabularReport) {
    ...
    const lines = [
      ...
      ...report.rows.map((r) =>
        report.columns.map((c) => esc(fmtCell(r[c.key]))).join(","),
      ),
    ];
  ...
  async sendExcel(res: Response, report: TabularReport) {
    ...
    if (report.subtitle)
      ws.addRow([report.subtitle]).font = {
        size: 10,
        color: { argb: "FF6B7280" },
      };
    ws.addRow([]);
    ...
    if (report.totals) {
      const totalRow = ws.addRow(
        report.columns.map(
          (c) =>
            report.totals![c.key] ?? (c === report.columns[0] ? "TOTAL" : ""),
        ),
      );
  ...
  sendPdf(res: Response, report: TabularReport) {
    ...
    for (const [idx, r] of report.rows.entries()) {
      const limit = report.columns.length > 6 ? 80 : 60;
      if (y > doc.page.height - limit) {
        doc.addPage();
        y = 40;
        drawHeader();
      }
      ...
      report.columns.forEach((c, i) => {
        doc.text(fmtCell(r[c.key]), x + 4, y + 5, {
          width: colWidths[i] - 8,
          ellipsis: true,
        });
        x += colWidths[i];
    ...
    doc
      ...
      .fillColor("#9CA3AF")
      .text(
        `Generated ${new Date().toISOString()} · AURA GEM ERP`,
        40,
        doc.page.height - 50,
      );
    doc.end();
