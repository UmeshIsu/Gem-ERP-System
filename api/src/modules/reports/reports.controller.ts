...
export class ReportsController {
  ...
  async report(
    ...
    if (q.format && tabular) {
      if (q.format === "csv") return this.exporter.sendCsv(res, tabular);
      if (q.format === "excel") return this.exporter.sendExcel(res, tabular);
      if (q.format === "pdf") return this.exporter.sendPdf(res, tabular);
    }
    res.json(data);
  }
}
