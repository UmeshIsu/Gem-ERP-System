...
const DOC_TYPES = ['application/pdf', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'text/csv'];
...
export class StorageService {
  ...
  async store(file: Express.Multer.File, folder: string): Promise<StoredFile> {
    ...
    const url = await this.persist(mainKey, mainBuffer, isImage ? 'image/webp' : file.mimetype);
    const thumbUrl = thumbBuffer && thumbKey ? await this.persist(thumbKey, thumbBuffer, 'image/webp') : undefined;
  ...
  private async persist(key: string, buffer: Buffer, _contentType: string): Promise<string> {
    if (this.driver === 's3') {
      // Production: plug AWS SDK v3 / Supabase client here using S3_* env vars.
      this.logger.warn('S3 driver configured but SDK integration not wired — falling back to disk');
    }
    const filePath = join(this.uploadDir, key);
    await mkdir(join(this.uploadDir, key.split('/').slice(0, -1).join('/')), { recursive: true });
