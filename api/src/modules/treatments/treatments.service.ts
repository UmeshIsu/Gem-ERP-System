...
export class TreatmentsService {
  ...
  getBatch(id: string) {
    return this.prisma.treatmentBatch.findUniqueOrThrow({
      where: { id },
      include: batchInclude,
    });
  }
  ...
  async createBatch(dto: CreateBatchDto, userId: string) {
    const stones = await this.prisma.stone.findMany({
      where: { id: { in: dto.stoneIds } },
    });
    if (stones.length !== dto.stoneIds.length)
      throw new BadRequestException("One or more stones not found");
  ...
  async updateBatch(id: string, dto: UpdateBatchDto, userId: string) {
    const before = await this.prisma.treatmentBatch.findUniqueOrThrow({
      where: { id },
    });
    if (before.status === BatchStatus.COMPLETED)
      throw new BadRequestException("A completed batch cannot be edited");
  ...
  async startBatch(id: string, userId: string) {
    const batch = await this.prisma.treatmentBatch.findUniqueOrThrow({
      where: { id },
      include: { stones: true },
    });
    if (batch.status !== BatchStatus.PENDING)
      throw new BadRequestException("Only a pending batch can be started");
    ...
    await this.prisma.$transaction(async (tx) => {
      await tx.treatmentBatch.update({
        ...
        data: {
          ...
          startAt: batch.startAt ?? new Date(),
        },
      });
  ...
  async completeBatch(id: string, dto: CompleteBatchDto, userId: string) {
    if (
      ![BatchStatus.COMPLETED, BatchStatus.FAILED].includes(dto.status as any)
    ) {
    ...
    await this.prisma.$transaction(async (tx) => {
      await tx.treatmentBatch.update({
        ...
        data: {
          ...
          remarks: dto.remarks ?? batch.remarks,
        },
      });
        ...
        if (result) {
          await tx.treatmentBatchStone.update({
            ...
            data: {
              ...
              weightAfterCt: result.weightAfterCt,
            },
          });
          if (result.weightAfterCt) {
            await tx.stone.update({
              where: { id: link.stoneId },
              data: { weightCt: result.weightAfterCt },
            });
          }
        }
  ...
  async addBatchImage(
    ...
    const image = await this.prisma.treatmentImage.create({
      data: { batchId: id, url, caption },
    });
    await this.audit.log({
      userId,
      action: "ADD_IMAGE",
  ...
  async addElectricProgress(
    id: string,
    dto: ElectricProgressDto,
    userId: string,
  ) {
    const run = await this.prisma.electricTreatment.findUniqueOrThrow({
      where: { id },
      include: { stone: true },
    });
    if (
      run.status !== ElectricStatus.RUNNING &&
      run.status !== ElectricStatus.PAUSED
    ) {
      throw new BadRequestException(
        "This electric treatment is already finished",
      );
    }
    ...
    const log = await this.prisma.$transaction(async (tx) => {
      const created = await tx.electricProgressLog.upsert({
        ...
        update: {
          ...
          colorImprovement: dto.colorImprovement,
          imageUrl: dto.imageUrl,
          notes: dto.notes,
        },
        create: { treatmentId: id, ...dto },
      });
      await tx.electricTreatment.update({
        where: { id },
        data: { completionPct: dto.completionPct },
      });
      await tx.stoneEvent.create({
        data: {
          stoneId: run.stoneId,
  ...
  async updateElectric(id: string, dto: UpdateElectricDto, userId: string) {
    const before = await this.prisma.electricTreatment.findUniqueOrThrow({
      where: { id },
    });
    ...
    const run = await this.prisma.$transaction(async (tx) => {
      const updated = await tx.electricTreatment.update({
        ...
        data: {
          ...dto,
          ...(dto.plannedWeeks && {
            estimatedFinish: addWeeks(before.startDate, dto.plannedWeeks),
          }),
        },
      });
      if (dto.status === ElectricStatus.COMPLETED) {
        ...
        await tx.notification.create({
          data: {
            ...
            body: `Stone treatment run finished after 100% progress`,
